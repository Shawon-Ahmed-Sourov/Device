...
class WineLauncher(QWidget):
    ...
    def build_ui(self):
        ...
        for r in (  # 1st-Col #2nd-Col #3rd-Col 
    ...
    def sel_exe(self):
        ...
        if not p: return
        self.exe = p  # Assigning the file path directly
        self.temp = os.path.join(os.path.dirname(p), ".wine_temp_overlay")  # Use the directory of the EXE
    ...
    def mk_temp(self):
        if os.path.exists(self.temp): return self.log.append(f"ℹ️ Temp Prefix already exists at {self.temp}")
    ...
    def del_temp(self):
        if not os.path.exists(self.temp): return self.log.append("ℹ️ No Temp Prefix found.")
    ...
    def inst_deps(self):
        if not self.base: return self.log.append("❌ Select a Base Prefix first.")
