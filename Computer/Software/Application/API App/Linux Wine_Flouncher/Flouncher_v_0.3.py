# Date : 19 OCT 2025
# Flouncher_v_0.3.py
# using : Tmpfs method inst-symlinking Base prefix
# yes a success : but slower, ConsumeRam more, may Crash
...
class WineLauncher(QWidget):
    ...
    def create_temp_prefix(self):
        ...
        # Use RAM-backed tmpfs
        tmp_dir = "/dev/shm" if os.path.exists("/dev/shm") else tempfile.gettempdir()
        self.temp_prefix = os.path.join(tmp_dir, f"wine_temp_{os.getpid()}")

        # Remove any old temp prefix
        ...
        try:
            # Copy skeleton from base prefix (minimal dirs: drive_c, system32)
            skeleton_dirs = ["drive_c", "dosdevices"]
            os.makedirs(self.temp_prefix, exist_ok=True)
            for d in skeleton_dirs:
                src = os.path.join(self.base_prefix, d)
                dst = os.path.join(self.temp_prefix, d)
                if os.path.exists(src):
                    shutil.copytree(src, dst, symlinks=True)
            self.log_output.append(f"✅ Temp Prefix created in RAM at {self.temp_prefix}")
            ...
            # fallback: full disk copy
            fallback_dir = os.path.join(self.game_dir, ".wine_temp_disk")
            try:
                if os.path.exists(fallback_dir):
                    shutil.rmtree(fallback_dir)
                shutil.copytree(self.base_prefix, fallback_dir, symlinks=True)
                self.temp_prefix = fallback_dir
                self.log_output.append(f"⚠️ Fallback: Disk-based temp prefix at {self.temp_prefix}")
            except Exception as e2:
                self.log_output.append(f"❌ Fallback failed: {e2}")
