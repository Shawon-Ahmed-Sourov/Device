...
class WineLauncher(QWidget):
    ...
    def __init__(self):
        ...
        from GUI import UIBuilder
        self.setWindowTitle("Wine EXE Launcher")
        self.resize(850, 450)
        
        # 1. State Management
        self.worker_thread, self.bprefix_path, self.exe_file, self.exe_path = None, None, None, None
    ...
    def on_modify_temp_changed(self, index):
        ...
        if index in mapping:
            self._start_task(
                            Prefix,  
                            (  mapping[index], self.exe_path, self.bprefix_path  ),
                            self.modify_temp
                        )
    ...
    def _start_task(self, worker_class, args, ui_element):
        ...
        self.worker_thread = worker_class(*args)    # 3. Initialize & Connect
        if hasattr(self.worker_thread, 'log'):    self.worker_thread.log.connect(self.log.append)
    
        def on_complete():                          # 4. Cleanup Logic
